...
def test(folder, gt_folder, end, csv_file, classes):
    if folder == gt_folder:
        images = [os.path.basename(x) for x in glob.glob(gt_folder + '/*_pred.nii.gz')]
    else:
        images = os.listdir(folder)
    images.sort()
    labels = images
    # labels = [os.path.basename(x) for x in glob.glob(gt_folder + '/*_gt.nii.gz')]
    # labels.sort()
    ...
    patients = Parallel(n_jobs=10)(
        delayed(parallel_test)(images[j], labels[j], folder, gt_folder,
                               end, classes) for j in range(len(labels)))
    ...
    with open(folder + end + '.csv', 'w') as outcsv:
        ...
        mean_final = np.nanmean(mean, axis=0)
        last = ['mean'] + list(mean_final)
...
def parallel_test(im, lb, test_dir, gt_dir, end, classes):
    dice = []
    precision = []
    recall = []
    # hausdorff = []
    ...
    # name = lb[:-10]
    name = im[:-12]
    im_path = os.path.join(test_dir, name + end + '.nii.gz')
    lb_path = os.path.join(gt_dir, name + '_gt.nii.gz')
