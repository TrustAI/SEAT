...
class AutoAttack():
    ...
    def run_standard_evaluation(self, bs=3):
        ...
        if os.path.isfile(results_file):# and False:
        ...
        _actual_dice_thresh = mean * self.dice_thresh
        robust_flags = clean_dice > _actual_dice_thresh
        ...
        if self.verbose:
            ...
            self.logger.log(
                txt.format(self.epsilon, _actual_dice_thresh, robust_acc))
        ...
        for attack in self.attacks_to_run:
            ...
            robust_flags = new_dice > _actual_dice_thresh
            robust_acc = torch.sum(robust_flags).item() / len(self.loader)
            txt = '\n Eps: {}, Dice ths: {}, Iters: {}'
            self.logger.log(
                txt.format(self.epsilon, _actual_dice_thresh, self.n_iter))
            ...
            for b_idx in robust_lin_idcs:
                ...
                if attack == 'apgd-ce':
                    self.apgd.loss = 'ce'
                    self.apgd.dice_thresh = _actual_dice_thresh
                    ...
                    self.fab.dice_thresh = _actual_dice_thresh
                    adv_curr = self.fab.perturb(x, y)

                elif attack == 'square':
                    self.square.seed = self.get_seed()
                    self.square.dice_thresh = _actual_dice_thresh
                    ...
                    delta = attack_pgd(
                        self.model, x, y, epsilon, self.device, epsilon / 4.,
                        iters=self.n_iter)
                    adv_curr = x + delta
    ...
    for _ in range(restarts):
        with torch.enable_grad():
            ...
            for _ in range(iters):
                ...
                d = clamp(delta + alpha * torch.sign(grad)*b_eps, -eps*b_eps, eps*b_eps)
                d = clamp(d, b_min - X, b_max - X)
                delta.data = d
                delta.grad.zero_()
