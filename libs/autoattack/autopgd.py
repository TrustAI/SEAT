...
class APGDAttack():
    ...
    def attack_single_run(self, x_in, y_in,b_min,b_max,b_eps):
        ...
        for _ in range(self.eot_iter):
            with torch.enable_grad():
                ...
                # print(loss_indiv.shape)
                # raise KeyError()
                loss = loss_indiv.sum()

            # 1 backward pass (eot_iter = 1)
            # cur_grad = torch.autograd.grad(loss, [x_adv])[0].detach()
            # print((cur_grad.abs() < 1e-5).sum())
            # print(cur_grad.numel())
            # # print((torch.autograd.grad(loss, [x_adv])[0].detach()))
            # raise KeyError()
        ...
        step_size = self.eps *b_eps* torch.ones(
            [x.shape[0], 1, 1, 1, 1]).to(self.device).detach()\
        ...
        for i in range(self.n_iter):
            ...
            with torch.no_grad():
                ...
                x_adv_1 = clamp(
                    torch.min(
                        torch.max(x_adv_1, x - self.eps*b_eps), x + self.eps*b_eps),
                    b_min,b_max)
                x_adv_1 = clamp(torch.min(
                    torch.max(
                        x_adv + (x_adv_1 - x_adv) * a + grad2 * (1 - a),
                        x - self.eps*b_eps), x + self.eps*b_eps),
                    b_min,b_max)
            ...
            for _ in range(self.eot_iter):
                with torch.enable_grad():
                    logits = self.model(x_adv)  # 1 forward pass (eot_iter = 1)
                    if self.loss == 'seg':
                        loss_indiv = criterion_indiv(logits, y, i)
                    else:
                        loss_indiv = criterion_indiv(logits, y)
                    loss = loss_indiv.sum()
        ...
        # print(torch.max((x_best_adv-x).abs()),torch.min((x_best_adv-x).abs()),b_eps)
    ...
    def perturb(self, x_in, y_in):
        ...
        b_min = torch.amin(x, [2, 3, 4], keepdim=True)
        b_max = torch.amax(x, [2, 3, 4], keepdim=True)
        b_eps = (b_max - b_min)
        # print(b_eps,b_eps*self.eps)
        ...
        for counter in range(self.n_restarts):
            ...
            if ind_to_fool.numel() != 0:
                ...
                _, results_curr, _, adv_curr = self.attack_single_run(
                    x_to_fool, y_to_fool,b_min,b_max,b_eps)
