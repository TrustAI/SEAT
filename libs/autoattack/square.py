...
class SquareAttack():
    ...
    def attack_single_run(self, x, y,b_min,b_max,b_eps):
        with torch.no_grad():
            c, d, h, w = x.shape[1:]
            n_features = c * d * h * w
            ...
            x_best = clamp(x + self.eps *b_eps* self.random_choice(
                [x.shape[0], c, 1, h, w]),b_min,b_max)
            ...
            for i_iter in range(self.n_queries):
                ...
                new_deltas[
                    ...
                    ] = 2. * self.eps *b_eps* self.random_choice([c, 1, 1, 1])

                x_new = x_best_curr + new_deltas
                x_new = torch.min(torch.max(x_new, x_curr - self.eps*b_eps),
                                  x_curr + self.eps*b_eps)
                # x_new = torch.clamp(x_new, 0., 1.)
                x_new = clamp(x_new,b_min,b_max)
    ...
    def perturb(self, x, y=None):
        ...
        for counter in range(self.n_restarts):
            ...
            if ind_to_fool.numel() != 0:
                ...
                _, adv_curr = self.attack_single_run(x_to_fool, y_to_fool,b_min,b_max,b_eps)
        ...
        # return adv_curr
        # print(torch.max((adv-x).abs()),torch.min((adv-x).abs()),b_eps)
        return adv
