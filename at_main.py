...
from settings import plan_experiment,get_kernels_strides
...
def main(rank, world_size, args):
    ...
    info, model_params = plan_experiment(
        ...
        args.fold, rank, out_root=args.root)

    # PATHS AND DIRS
    # args.save_path = os.path.join(
    #     info['output_folder'], args.name')
    args.save_path = os.path.join(
        info['output_folder'], args.name)
    images_path = os.path.join(args.save_path, 'volumes')
    if args.adv:
        adv_path = os.path.join(args.save_path, f'new_autoattack{args.eps}')
    ...
    if args.pretrained is not None:
        load_path = os.path.join(
            'Results', tasks[args.task], args.pretrained)
    ...
    if args.model == 'rog':
        ...
        model = SegResNet(
                ...
                blocks_up=[1, 1, 1],
                init_filters=16,
                in_channels=model_params['modalities'],
                out_channels=model_params['classes'],
                dropout_prob=0.2).to(rank)
    ...
    elif args.model == 'dyunet':
        kernel_size, strides = get_kernels_strides(args.task)
        model = DynUNet(
            spatial_dims=3,
            in_channels=model_params['modalities'],
            out_channels=model_params['classes'],
            ...
            deep_supr_num=deep_supr_num[args.task]      
        ).to(rank)
    ...
    if training or args.ft:
        ...
        optimizer = optim.Adam(
            ddp_model.parameters(), lr=args.lr, amsgrad=True,
            weight_decay=1e-5)
        # annealing = optim.lr_scheduler.CosineAnnealingLR(optimizer, T_max=args.epochs)
        ...
        # annealing = optim.lr_scheduler.MultiStepLR(optimizer, milestones=[args.epochs * 1 / 2, args.epochs * 14/15], gamma=0.1)
    ...
    train_loader = DataLoader(
        ...
        num_workers=8, collate_fn=train_collate, pin_memory=True)
    val_loader = DataLoader(
        val_dataset, sampler=None, batch_size=info['test_batch'],
        num_workers=8, collate_fn=val_collate, pin_memory=True)
    test_loader = DataLoader(
        test_dataset, sampler=None, shuffle=False, batch_size=info['test_batch'], num_workers=1)
    ...
    def adjust_adv_iteration(cur_epoch, info, args, mag):
        if cur_epoch == args.check_mag:
            info['iter'] += 1
        elif cur_epoch == args.check_mag + 1:
            info['threshold'] = mag * args.gamma
        elif cur_epoch % args.check_mag == 0:
            if (mag > info['threshold'] and
                info['iter'] < args.adv_iters):
                info['threshold'] = args.gamma * mag
                info['iter'] += 1
            elif (mag < info['threshold']/args.gamma and
                info['iter'] > 2):
                info['threshold'] = args.gamma * mag
                info['iter'] -= 1
        else:
    ...
    if training:
        ...
        if args.adv_trainer == 'free':
            ...
            info['iter'] = args.adv_iters
        elif args.adv_trainer == 'seat':
            at_trainer = trainer.seat_train 
        else:
    ...
    if args.adv:
        adversary = AutoAttack(
            ddp_model.forward, adv_loader, 
            dice_thresh= 1./model_params['classes'], 
            device=rank,
            n_target_classes=info['classes']-1, eps=args.eps,
            n_iter=args.adv_iters, im_path=adv_path,
            log_path=os.path.join(args.save_path, 
                f'log_autoattack_{args.eps}_{args.adv_iters}'),
            model_name=args.load_model)
        attack = ['pgd-ce', 'apgd-ce', 'square'] 
...
if __name__ == '__main__':
    ...
    parser.add_argument('--model', type=str, default='rog', choices = ['rog', 'srn', 'unet', 'dyunet'],
                        help='Model architecture (default rog)')
    parser.add_argument('--fold', type=str, default=0,
                        help='Which fold to run. Value from 0 to 4')
    parser.add_argument('--root', type=str, default='Results',
                        help='root of outputs')
